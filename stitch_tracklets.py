    ...
    for i, (k, v) in enumerate(previous_instances.items()):
        ...
        for j, (k1, v1) in enumerate(current_instances.items()):
            if v1['class'] == v['class'] and k1 not in overlaps:
                ...
                if k1 in current_instances_prev:
                    cost_center = min(cost_center, euclidean_dist(
                        current_instances_prev[k1]['kalman_bbox'], v['kalman_bbox']))
                if cost_center > 5:
                    cost_center = 1e8
...
def associate_instances_overlapping_frames(previous_ins_label, current_ins_label):
    ...
    previous_instance_ids, c_p = np.unique(
        previous_ins_label, return_counts=True)
    current_instance_ids, c_c = np.unique(
        current_ins_label, return_counts=True)

    previous_instance_ids = [x for i, x in enumerate(
        previous_instance_ids) if c_p[i] > 25]
    current_instance_ids = [x for i, x in enumerate(
        current_instance_ids) if c_c[i] > 50]
    ...
    for i, p_id in enumerate(previous_instance_ids[1:]):
        ...
        for j, c_id in enumerate(current_instance_ids[1:]):
            ...
            union = np.sum(previous_ins_label == p_id) + \
                np.sum(current_ins_label == c_id) - intersection
...
def main(FLAGS):
    ...
    association_weights = [FLAGS.iou_3d,
                           FLAGS.iou_2d, FLAGS.center, FLAGS.feature]
    ...
    inv_learning_map = np.zeros(
        (np.max([k for k in inv_learning_map_doc.keys()]) + 1), dtype=np.int32)
    ...
    for sequence in test_sequences:
        calib = parse_calibration(os.path.join(
            dataset, "sequences", '{0:02d}'.format(sequence), "calib.txt"))
        poses_f64 = parse_poses(os.path.join(
            dataset, "sequences", '{0:02d}'.format(sequence), "poses.txt"), calib)
        poses.append([pose.astype(np.float32) for pose in poses_f64])

    for poses_seq, sequence in zip(poses, test_sequences):
        point_names = []
        point_paths = os.path.join(
            dataset, "sequences", '{0:02d}'.format(sequence), "velodyne")
        ...
        if not os.path.exists(os.path.join(save_path, 'sequences', '{0:02d}'.format(sequence))):
            os.makedirs(os.path.join(save_path, 'sequences',
                        '{0:02d}'.format(sequence)))

        if not os.path.exists(os.path.join(save_path, 'sequences', '{0:02d}'.format(sequence), 'predictions')):
            os.makedirs(os.path.join(save_path, 'sequences',
                        '{0:02d}'.format(sequence), 'predictions'))
        ...
        for idx, point_file in zip(range(len(point_names)), point_names):
            ...
            sem_path = os.path.join(
                prediction_path, '{0:02d}_{1:06d}.npy'.format(sequence, idx))
            ins_path = os.path.join(
                prediction_path, '{0:02d}_{1:06d}_i.npy'.format(sequence, idx))
            fet_path = os.path.join(
                prediction_path, '{0:02d}_{1:06d}_f.npy'.format(sequence, idx))
            ...
            things = (label_sem_class < things_label_max) & (
                label_sem_class > 0)
            ...
            for ins_id in ins_ids:
                ...
                (values, counts) = np.unique(
                    label_sem_class[ids], return_counts=True)
            ...
            if idx > 0:
                for i in range(1, n_test_frames):
                    ...
                    prev_inst_path = os.path.join(
                        prediction_path, '{0:02d}_{1:07d}_{2}_i.npy'.format(sequence, idx-i, idx))
                    prev_sem_path = os.path.join(prediction_path,
                                                 '{0:02d}_{1:07d}.npy'.format(sequence, idx - i))

                    fet_path = os.path.join(
                        prediction_path, '{0:02d}_{1:07d}_{2}_f.npy'.format(sequence, idx - i, idx))
                    ...
                    if os.path.exists(fet_path):
                        features = np.load(
                            fet_path, allow_pickle=True).tolist()
                    else:
                        features = {}
                        for ins_id in ins_ids:
                            features[ins_id] = torch.from_numpy(
                                np.zeros((1, 1)))
                    ...
                    association_costs, associations = associate_instances_overlapping_frames(
                        prev_inst_orig * things, prev_inst * things)
                    ...
                    prev_point_path = os.path.join(dataset, "sequences", '{0:02d}'.format(
                        int(sequence)), "velodyne", '{0:06d}.bin'.format(idx-i))
                    #pose = poses[0][idx-i]
                    frame_points = np.fromfile(
                        prev_point_path, dtype=np.float32)
                    points = frame_points.reshape((-1, 4))
                    hpoints = np.hstack(
                    ...
                    points = torch.from_numpy(points)
                    projections = do_range_projection(
                    ...
                    for ins_id in ins_ids:
                        ...
                        (values, counts) = np.unique(
                            prev_sem[ids], return_counts=True)
                        inst_class = values[np.argmax(counts)]
            ...
            if len(prev_instances.keys()) > 0:
                ...
                for (new_id, prev_id) in overlaps.items():
                    ...
                    prev_instances[prev_id]['kalman_bbox'] = torch.from_numpy(
                        prev_instances[prev_id]['tracker'].get_state()).float()
            ...
            for sem_id in np.unique(label_sem_class):
                if sem_id < 1 or sem_id > 8:
                    valid_ind = np.argwhere(
                        (label_sem_class == sem_id) & (ins_preds == 0))[:, 0]

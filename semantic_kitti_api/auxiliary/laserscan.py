...
class SemLaserScan(LaserScan):
    ...
    def __init__(self, nclasses, sem_color_dict=None, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0, known=True, unknown=True, dataset="semantic-kitti", task_set=-1):
        ...
        if dataset == "semantic-kitti":
            if self.task_set == -1:
                ...
                raise ValueError(
                    "Task set not supported, choose among: -1, 1, 2")
            ...
            raise ValueError(
                "Dataset not supported, choose among: semantic-kitti, kitti-360")
        ...
        self.inst_color_lut_known[:, [
            0, 2]] = self.inst_color_lut_known[:, [2, 0]]  # rgb to bgr
        self.inst_color_lut_unknown[:, [
            0, 2]] = self.inst_color_lut_unknown[:, [2, 0]]  # rgb to bgr
    ...
    def set_label(self, label):
        ...
        if label.shape[0] == self.points.shape[0]:
            ...
            self.sem_label[self.sem_label < 0] = 0
    ...
    def colorize(self):
        ...
        known_mask = np.logical_and(
            self.sem_label != 100, np.in1d(self.sem_label, self.things))
        ...
        if self.known:
            self.inst_label_color[known_mask] = self.inst_color_lut_known[(
                inv_instance[self.inst_label[known_mask]]) % self.max_inst_id]

        if self.unknown:
            self.inst_label_color[unknown_mask] = self.inst_color_lut_unknown[(
                inv_instance[self.inst_label[unknown_mask]]) % self.max_inst_id]
    ...
    def do_label_projection(self):
        ...
        self.proj_inst_color[mask] = self.inst_color_lut_known[self.inst_label[self.proj_idx[mask]] % self.max_inst_id]

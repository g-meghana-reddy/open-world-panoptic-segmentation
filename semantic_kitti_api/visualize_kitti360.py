...
if __name__ == '__main__':
    ...
    FLAGS.sequence = '2013_05_28_drive_{0:04d}_sync'.format(
        int(FLAGS.sequence))

    # does sequence folder exist?
    scan_paths = os.path.join(
        FLAGS.dataset, 'data_3d_raw', FLAGS.sequence, 'velodyne_points', 'data')
    gt_paths = os.path.join(FLAGS.dataset, 'data_3d_raw',
                            FLAGS.sequence, 'velodyne_points_labeled_10cm')
    ...
    if not FLAGS.ignore_semantics:
        if FLAGS.predictions is not None:
            label_paths = os.path.join(
                FLAGS.predictions, "sequences", "{:02d}".format(seq_id), "predictions")
        else:
            label_paths = gt_paths

        # populate the pointclouds
        file_ids = set(int(vf[:-6])
                       for vf in os.listdir(label_paths) if vf.endswith('.label'))
    ...
    if FLAGS.ignore_semantics:
        ...
        if FLAGS.visu == 0:
            ...
            scan = SemLaserScan(nclasses, color_dict, project=True, unknown=False,
                                known=True, dataset="kitti-360", task_set=FLAGS.task)

        if FLAGS.visu == 1:
            # 2. Unknown instances with distinct id colors
            scan = SemLaserScan(nclasses, color_dict, project=True, unknown=True,
                                known=False, dataset="kitti-360", task_set=FLAGS.task)

...
class SemanticKittiRawDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', balance_classes=True, seqential_batch = False, return_unknowns = False):
        ...
        self.sequences = []
        sequences = [i for i in listdir(self.path) if isdir(os.path.join(self.path,i))]
        for seq in sequences:
            [self.sequences.append(join(seq,i)) for i in listdir(join(self.path, seq)) if i.endswith('_sync')]     
        ...
        for seq in self.sequences:
            velo_path = join(self.path, seq, 'velodyne_points', 'data')
        ...
        with open(config_file, 'r') as stream:
            ...
            if self.return_unknowns:
                ...
                unknown_label = max(learning_map.values())
                for k, v in learning_map.items():
    ...
    def __getitem__(self, batch_i):
        ...
        while True:
            ...
            while num_merged < self.config.n_frames and f_ind - f_inc >= 0:
                ...
                velo_file = join(seq_path, 'velodyne_points', 'data', self.frames[s_ind][f_ind - f_inc] + '.bin')
...
class SemanticKittiRawSamplerTest(Sampler):
    """Sampler for SemanticKitti for testing"""

    def __init__(self, dataset: SemanticKittiRawDataset):
...
class SemanticKittiRawSampler(Sampler):
...
class SemanticKittiRawCustomBatch:
...
def SemanticKittiRawCollate(batch_data):
    return SemanticKittiRawCustomBatch(batch_data)

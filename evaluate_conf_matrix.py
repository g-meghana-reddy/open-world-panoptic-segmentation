...
def parse_args():
    ...
    parser.add_argument( '-eval', "--evaluate", dest='evaluate', action="store_true", default=False)
    parser.add_argument( '-conf', "--confusion", dest='confusion', action="store_true", default=False)
...
if __name__ == '__main__':
    ...
    if args.semantic_kitti:
        ...
        xlabels = ['car', 'truck', 'person', 'road', 'sidewalk', 'building' ,'fence', 'vegetation','terrain','bicycle' ,'bus', 'motorcycle', 'on-rails', 'other-vehicle','parking', 'other-ground', 'other-struct', 'trunk', 'pole' ,'traffic-sign','other-object', 'mov-on-rails', 'mov-bus', 'mov-other-veh']
    ...
    filename = 'confusion_matrices/{}/TS{}/labels_{}.npz'.format(dataset_name, args.task_set, args.seq)
    ...
    if args.evaluate:
        ...
        # Save true_mapped_labels, predictions, val_label_values, unknown_label_values
        
        np.savez(filename,
                true_mapped_labels=true_mapped_labels,
                true_unmapped_labels=true_unmapped_labels,
                predictions=predictions,
                val_label_values=val_label_values,
                unknown_label_values=unknown_label_values,
                num_classes=num_classes,
                num_unknown_classes=num_unknown_classes,
                label_names=test_dataset.label_names,
                unknown_label_names=test_dataset.unknown_label_names
                )
    ...
    if args.confusion:
        ...
        if args.kitti360:
            filename = 'confusion_matrices/{}/TS{}/labels_{}.npz'.format(dataset_name, args.task_set, 3)
            labels = dict(np.load(filename, allow_pickle=True))

            true_mapped_labels = labels['true_mapped_labels']
            true_unmapped_labels = labels['true_unmapped_labels']
            predictions = labels['predictions']
            val_label_values = labels['val_label_values']
            unknown_label_values = labels['unknown_label_values']
            num_classes = labels['num_classes']
            num_unknown_classes = labels['num_unknown_classes']

            for seq in [5, 7]:
                filename = 'confusion_matrices/{}/TS{}/labels_{}.npz'.format(dataset_name, args.task_set, 3)
                labels = dict(np.load(filename, allow_pickle=True))

                true_mapped_labels = np.concatenate([true_mapped_labels, labels['true_mapped_labels']])
                true_unmapped_labels = np.concatenate([true_unmapped_labels, labels['true_unmapped_labels']])
                predictions = np.concatenate([predictions, labels['predictions']])
                

        else:
            filename = 'confusion_matrices/{}/TS{}/labels_{}.npz'.format(dataset_name, args.task_set, 8)
            labels = dict(np.load(filename, allow_pickle=True))
        ...
        if args.task_set == -1:
            ...
            y_labels = np.array(label_names)[1:]
            x_labels = y_labels
            ...
            plt.show()
            plt.savefig('confusion_matrices/updated_confusion_matrix_ts{}_{}_balanced.png'.format(args.task_set, dataset_name))
            ...
            # unk_to_known_y_labels = np.array(label_names)[1:-1]
            # unk_to_known_x_labels = np.concatenate([unk_to_known_y_labels, unknown_label_names[:-1]])
            # unk_to_known_y_labels = np.array(label_names)[1:]

            plt.rcParams['font.family'] = 'serif'
            plt.rcParams['font.serif'] = ['Times New Roman'] + plt.rcParams['font.serif']
            plt.figure(figsize = (20,10))
            b = sns.heatmap(unk_to_known_conf, xticklabels=xlabels, yticklabels=ylabels, cmap='Blues', robust=True, square=True, cbar_kws={"shrink": shrink})
            # b.set_yticklabels(b.get_yticks(), size = 8)
            # b.set_xticklabels(b.get_xticks(), size = 8)
            
            
            plt.xlabel('Groundtruth Class', fontsize=10)
            plt.ylabel('Detected Class', fontsize=10)
            # plt.subplots_adjust(bottom=0.35)
            # plt.subplots_adjust(left=0.35)
            plt.show()
            plt.savefig('confusion_matrices/updated_extended_confusion_matrix_ts{}_{}_balanced.png'.format(args.task_set, dataset_name))
            ...
            # plt.subplots_adjust(bottom=0.15)
            # plt.show()
            # plt.savefig('confusion_matrices/updated_normal_confusion_matrix_ts{}_{}_balanced.png'.format(args.task_set, dataset_name))
